...
To prevent unprivileged users (like Contributors) from using this shortcode to have privileged users (like Admins) accidentally execute arbitrary javascript via HTML event attributes (like `onclick`, `onmouseover`, etc), you can not set your `attr` option to an event attribute and any passed shortcode attributes that are event attributes will be ignored.
